...
const Notes = () => {
  ...
  return (
    <section id="notes" className="py-20 bg-bg-alt">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="flex justify-center mb-8">
          <div className="glass-panel p-1 rounded-xl shadow-sm flex gap-1">
            {[
        ...
        <div className="flex justify-center mb-10">
          <div className="relative w-full max-w-md">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 rounded-2xl border border-border bg-input-bg text-text placeholder:text-muted focus:border-accent focus:ring-4 focus:ring-accent/10 outline-none transition-all shadow-sm"
              value={searchQuery}
                ...
                chaptersList.map((chapter) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="glass-panel rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden group flex flex-col h-fit"
                    >
                      <div className="p-6 flex-grow flex flex-col relative">
                        ...
                        <h3 className="text-xl font-bold text-heading mb-4 leading-tight group-hover:text-accent transition-colors relative z-10">
                          {chapter}
                        </h3>

                        <button
                          onClick={() => toggleChapter(chapter)}
                          className="w-full py-2 px-4 bg-surface-hover hover:bg-surface text-muted font-bold rounded-xl flex items-center justify-between border border-border transition-all duration-300 relative z-10"
                        >
                        ...
                        <AnimatePresence>
                          {isExpanded && (
                            <motion.div
                              ...
                              transition={{ duration: 0.3 }}
                              className="overflow-hidden mt-4 pt-4 border-t border-border space-y-4 relative z-10 text-left"
                            >
                              {(() => {
                                  ...
                                  ([med, files]) => {
                                    ...
                                    return (
                                      <div key={med} className="space-y-2">
                                        <div className="text-[10px] font-bold text-muted uppercase tracking-wider pl-1 mb-1">
                                          {med === "EM"
                                            ? "English Medium (EM)"
                                            : "Gujarati Medium (GM)"}
                                        </div>
                                        {files.map((note) => (
                                          <div
                                            key={note.id}
                                            className="flex items-center justify-between p-3 bg-accent/5 hover:bg-accent/10 rounded-xl border border-border/50 transition-all duration-200 group/file"
                                          >
                                            <div className="flex items-center gap-2 max-w-[70%]">
                                              ...
                                              />
                                              <span className="text-xs font-semibold text-text truncate" title={note.file_name || getCleanFilename(note.file_url)}>
                                                {note.file_name ||
                                            ...
                                            <button
                                              ...
                                              className="p-2 bg-surface-solid text-muted hover:text-accent hover:bg-accent/10 rounded-lg transition-all group-hover/file:border-accent border border-border"
                                              title="Download PDF"
                                            >
                                              <Download size={14} />
                                            </button>
